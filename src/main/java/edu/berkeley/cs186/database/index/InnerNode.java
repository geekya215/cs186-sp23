...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        return child.put(key, rid).flatMap(this::spilt);
    }

    private Optional<Pair<DataBox, Long>> spilt(Pair<DataBox, Long> p) {
        DataBox k = p.getFirst();
        Long pageNum = p.getSecond();
        int idx = numLessThan(k, keys);
        ...
        if (keys.size() <= order * 2) {
            ...
            sync();
            return Optional.of(new Pair<>(pushUpKey, innerNode.getPage().getPageNum()));
        }
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        Optional<Pair<DataBox, Long>> rootBulkLoad = rightMostChild.bulkLoad(data, fillFactor);

        if (rootBulkLoad.isPresent()) {
            Optional<Pair<DataBox, Long>> spilt = spilt(rootBulkLoad.get());
            if (spilt.isPresent()) {
                return spilt;
            } else {
